...
Reference metrics from the tutorial notebook (10% sample, `chunk_size=1000`, `chunk_overlap=200`, `BAAI/bge-small-en-v1.5`):

### Retrieval

| Retriever | Hit@3 | MRR | nDCG@5 |
|---|---|---|---|
| Similarity (k=1) | 1.000 | 1.000 | 1.000 |
| Similarity (k=3) | 1.000 | 1.000 | 1.000 |
| MMR (λ=0.5) | 0.800 | 0.900 | 0.854 |
| Score threshold 0.4 | 1.000 | 0.900 | 0.906 |

### Generation

| Metric | Value | Interpretation |
|---|---|---|
| Perplexity (GPT-2) | 35.52 | "Good" — fluent and predictable |
| BERTScore F1 (vs DistilGPT2) | 0.438 | Moderate semantic overlap |
| BERTScore F1 (vs source) | 0.475 | Moderate factual overlap |
| Semantic faithfulness | 0.466 | Moderate grounding |
| Answer relevance (semantic) | 0.172 | Low — model drifts off-question |
| NLI faithfulness (RoBERTa-MNLI) | 0.256 | Weak entailment from context |

**Interpretation.** Retrieval is strong; the bottleneck is *generation*. The model grounds itself loosely in the retrieved chunks and often over-answers or drifts. This is exactly the failure mode the survey warns about: **retrieval quality alone does not guarantee faithful generation**. Improvements to try:

1. Lower `temperature` from `0.35` to `0.1`.
2. Tighten the RAG prompt to force "I don't know" when context is insufficient.
3. Increase `retrieval.k` from 3 to 5.
4. Try the `structured` prompt (forces shorter, more focused output).

---

## Extending the Pipeline

The codebase is designed to be extended without touching the core.

### Add a new loader

1. Implement a function in `src/rag_pipeline/ingestion/loaders.py`:

   ```python
   def load_parquet(path, **kw):
       from langchain_community.document_loaders import ParquetLoader
       return ParquetLoader(str(path), **kw).load()
   ```

2. Register it in `_LOADER_DISPATCH`:

   ```python
   _LOADER_DISPATCH = { ..., "parquet": load_parquet }
   ```

3. Use it in `configs/default.yaml`:
...
Same pattern in `splitters.py` — add a `_build_markdown(cfg)` function and register it under `_BUILDERS`.

### Add a new embedding provider

Add a function in `embeddings/factory.py` and register it under `_PROVIDERS`. The rest of the pipeline reads the provider by name from the config.

### Add a new vector store

Add a `(build_fn, load_fn)` pair to `_BUILDERS` in `vectorstores/factory.py`.

### Add a new prompt

Add a template constant to `generation/prompts.py` and register it in `PROMPT_REGISTRY`. It is then callable via `pipe.query(..., prompt_name="my_new_prompt")`.
